...
void nlcd_SendByte(char mode,unsigned char c)
{
    ...
    for(unsigned char i=0;i<8;i++)
    {
		...
		_delay_us(270);	// *****!!!!! Минимальная задержка, при которой работает мой LCD-контроллер
...
void nlcd_Rect (unsigned char x1, unsigned char y1, unsigned char x2, unsigned char y2, unsigned char fill, unsigned char pixel_mode)
{
	...
	else		// Без заливки
	{
		...
		nlcd_Line(x1, y1, x1, y2, pixel_mode);
		nlcd_Line(x2, y1, x2, y2, pixel_mode);
	}
}
...
//				PIXEL_OFF - для выключения пиксела
//				PIXEL_INV - для инверсии пиксела
//  picture: указатель на массив с монохромной картинкой
void nlcd_Pict  (unsigned char x, unsigned char y, unsigned char * picture)
{
	unsigned char xs=picture[0];
	unsigned char ys=picture[1];
	for (unsigned char i=0; i<ys;i++)
	{
		for (unsigned char j=0; j<xs;j++)
		{
			nlcd_Pixel(x,y,PIXEL_ON);
		}
	}
}
